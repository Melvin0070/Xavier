...
class MultiStepFormV4 {
    ...
    async handleFormCompletion() {
        ...
        try {
            ...
            this.globalUserPrompt = finalJobData.user_prompt || null;
    ...
    buildFinalJobData() {
        ...
        const baseData = {
            user_id: this.userId,
            user_prompt: this.extractUserPrompt(),
            blueprint: this.blueprint,
            sector: this.sector,
            company: this.company,
    ...
    /**
     * Derive user prompt from collected answers
     */
    extractUserPrompt() {
        if (!Array.isArray(this.allAnswers)) {
            return null;
        }

        const promptAnswer = this.allAnswers.find(answer => {
            const questionText = answer?.question || '';
            return /prompt|description|summary/i.test(questionText) && answer?.payload?.textInput;
        });

        if (promptAnswer?.payload?.textInput) {
            return promptAnswer.payload.textInput;
        }

        const firstTextInput = this.allAnswers.find(answer => answer?.payload?.textInput);
        return firstTextInput?.payload?.textInput || null;
    }
