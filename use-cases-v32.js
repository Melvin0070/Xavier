    ...
    const API = {
      ...
      async uploadUseCase(file, name, description, dataSourcesEnabled = false, iconPreference = 'white_icon') {
        if (!state.userId) {
      ...
      async fetchGraphTableData(templateId) {
        ...
        try {
          ...
          console.log('[API] Fetched', legacyItems.length, 'chart/table items (legacy format)');
          return {
            format: 'legacy',
      ...
      async createPromptInput(userId, useCaseId, variableType, customVariableName, displayOrder) {
        const response = await this.request(CONFIG.API.PROMPT_INPUTS_URL, {
          method: 'POST',
          body: JSON.stringify({
            action: 'create',
      ...
      async confirmDelete() {
        ...
        try {
          ...
          state.setUseCases(state.useCases.filter(uc => 
            String(uc.id || uc.use_case_id) !== String(idToDelete)
          ));
          UI.renderGrid();
          this.closeDelete();
    ...
    const Form = {
      ...
      async submit(e) {
        ...
        const selectedIconInput = document.querySelector('input[name="wfuc-icon-style"]:checked');
        
        const name = nameInput?.value || '';
        const description = descInput?.value || '';
        const externalDataEnabled = externalDataToggle?.checked || false;
        const iconPreference = selectedIconInput?.value || 'white_icon';
        
        try {
          btn.textContent = 'Uploading...';
          Toast.info('Upload started', 'Your presentation is uploading');
          
          const result = await API.uploadUseCase(state.selectedFile, name, description, externalDataEnabled, iconPreference);
    ...
    // ─── Global handlers called from HTML inline event attributes ───
    ...
    const DataSourceConfig = {
      ...
      getConfiguredCount() {
        ...
        for (const num of state.slideOrder) {
          ...
          for (const el of slide.elements) {
            ...
            if (!this.isSourceConfigured(sel)) continue;
            count++;
          }
        }
        return count;
      ...
      isSlideComplete(slideNum) {
        ...
        return slide.elements.every(el => {
          const sel = state.elementSelections[el.elementKey];
          if (!sel || !sel.source) return false;
          if (!this.isSourceConfigured(sel)) return false;
          return true;
        });
      },
        ...
        if (container) {
          container.innerHTML = `
            <div style="text-align:center;padding:60px 40px;color:#64748b">
              <div style="display:inline-flex;align-items:center;justify-content:center;width:56px;height:56px;background:rgba(var(--primary-rgb),0.08);border-radius:50%;margin-bottom:20px">
      ...
      buildElementRow(el, slide) {
        ...
        if (el.type === 'image') {
          options = [
            ...
            { value: 'white_icon', label: '⬜ White icon' },
            { value: 'black_icon', label: '⬛ Black icon' },
      ...
      buildReferenceSelect(el, slide) {
        ...
        if (sameSlideElements.length === 0) {
          ...
          msg.style.cssText = 'padding:12px 16px;color:#94a3b8;font-size:13px;font-style:italic;';
          msg.textContent = 'No other elements on this slide to reference';
        ...
        refSelect.onchange = () => {
          state.elementSelections[el.elementKey].reference = refSelect.value || null;
      ...
      buildPromptTextarea(el) {
        ...
        textarea.placeholder = 'Describe what data to generate for this element...';
        textarea.rows = 3;
      ...
      buildAiImagePrompt(el) {
        ...
        note.className = 'wfuc-ds-ai-prompt-note';
        note.textContent = 'Optional: describe what image to generate';

        const textarea = document.createElement('textarea');
        textarea.className = 'wfuc-ds-gen-prompt';
        textarea.placeholder = 'e.g. "A minimalist icon of a rocket launching, flat style..."';
        textarea.rows = 2;
        textarea.value = sel.prompt || '';

        const debouncedPrompt = Utils.debounce(() => {
          const existing = state.elementSelections[el.elementKey] || {};
          state.elementSelections[el.elementKey] = { ...existing, prompt: textarea.value };
          this.onSelectionChanged();
        }, 400);
      ...
      buildUserInputSelect(el) {
        ...
        if (cachedInputs.length > 0) {
          ...
          label.className = 'wfuc-ds-userinput-label';
          label.textContent = 'Insert variable';
          wrap.appendChild(label);

          const pillsRow = document.createElement('div');
          pillsRow.className = 'wfuc-ds-userinput-pills';
          ...
          cachedInputs.forEach(input => {
            ...
            const pill = document.createElement('button');
            pill.type = 'button';
            pill.className = 'wfuc-pv-pill-btn';
            pill.textContent = `{${varName}}`;
            pill.title = `Insert {${varName}} at cursor`;

            pill.addEventListener('mousedown', (e) => {
              e.preventDefault();
              this._insertPillIntoActiveEditor(wrap, varName);
            });
          ...
          msg.textContent = 'No variables defined yet. Add variables in Step 3.';
          wrap.appendChild(msg);
        }
        ...
        editor.setAttribute('data-placeholder',
          cachedInputs.length > 0
            ? 'Type your prompt\u2026 press { to insert a variable'
        ...
        const openAuto = () => {
          ...
          menuOpen = true;
          autoActiveIndex = 0;
          triggerRange = this._saveSelection();
        ...
        const insertFromMenuByIndex = (idx) => {
          const input = cachedInputs[idx];
          if (!input) return;
          const varName = input.variable_type === 'Custom'
            ? (input.custom_variable_name || 'Custom')
            : input.variable_type;
        ...
        editor.addEventListener('keydown', (e) => {
          ...
          if (e.key === 'ArrowDown') {
            ...
            e.preventDefault();
            if (autoActiveIndex >= 0) insertFromMenuByIndex(autoActiveIndex);
      ...
      _createPillElement(varName) {
        const pill = document.createElement('span');
        pill.className = 'wfuc-seg-pill';
      ...
      _insertPillFromAutocomplete(editor, savedRange, varName) {
        editor.focus();
        const sel = window.getSelection();
        ...
        if (savedRange) {
          ...
          // Move start back one character to capture the '{'
          if (range.startContainer.nodeType === Node.TEXT_NODE && range.startOffset > 0) {
            range.setStart(range.startContainer, range.startOffset - 1);
          }
          sel.removeAllRanges();
          sel.addRange(range);
          // Also select any additional text typed after '{'
          const currentRange = sel.getRangeAt(0);
          const textNode = currentRange.startContainer;
          if (textNode.nodeType === Node.TEXT_NODE) {
            const textAfterTrigger = textNode.textContent.substring(currentRange.startOffset);
            const closingIdx = textAfterTrigger.indexOf('}');
            if (closingIdx >= 0) {
              currentRange.setEnd(textNode, currentRange.startOffset + closingIdx + 1);
            }
          }
        }
      ...
      _renderAutoMenu(menu, inputs, activeIndex, onSelect) {
        menu.innerHTML = '';
        inputs.forEach((input, idx) => {
          ...
          item.className = `wfuc-autocomplete-item ${idx === activeIndex ? 'wfuc-active' : ''}`;
      ...
      quickApply(sourceType) {
        ...
        slide.elements.forEach(el => {
          state.elementSelections[el.elementKey] = { source: sourceType };
        });
