...
const API = {
  ...
  async uploadUseCase(file, name, description, dataSourcesEnabled) {
    if (!state.userId) {
  ...
  async submit(e) {
    ...
    try {
      ...
      if (externalDataEnabled) {
        ...
        Toast.info('Upload complete', 'We are preparing the data source setup');
        state.startPolling();
      } else {
        Modal.closeAdd();
...
const DataSourceConfig = {
  ...
  async open(userId, templateId) {
    ...
    try {
      ...
      // No pre-selection; require explicit user choice for each item
